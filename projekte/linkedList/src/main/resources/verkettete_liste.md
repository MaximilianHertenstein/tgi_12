...
Implementiere die Methode `subList(int fromIndex, int toIndex)` in `ImmutableLinkedList`, die eine `List<String>` mit den Elementen im Bereich `[fromIndex, toIndex]` (inklusive) zurückgibt. Bei ungültigen Indizes solle eine `IndexOutOfBoundException` geworfen werden.
